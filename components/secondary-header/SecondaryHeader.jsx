...
const SecondaryHeader = () => {
  ...
  console.log(urlCategoryIdd,"urlCategoryIdd")
  ...
  ] = useFetch(`sidebarfilter?category=${urlCategoryIdd||check}`, {
    method: 'get',
  })
  const [
    getCategories,
  ...
  useEffect(() => {
    setCountry(countryFromRedux ? countryFromRedux : 'usa')
  }, [])
  ...
  const getUrlCategoryIdFromLocalStorage = () => {
    return localStorage.getItem('urlCategoryId');
  };
  useEffect(() => {
    // Retrieve urlCategoryId from local storage
    const categoryId = getUrlCategoryIdFromLocalStorage();
    // Set urlCategoryId in the check state
    setCheck(categoryId);
  }, []);
  ...
  useEffect(() => {
    ...
    if (
      ...
      let urlCategoryId = allCategories[category0]?.airtabelId ?? check
      ...
      let collectionIdToUse =
        ...
        localStorage.setItem('urlCategoryId', urlCategoryId);
      if (category0) {
                    ...
                    >
                      ...
                      {allCategories && (
                        <div
                          ...
                          style={{
                            top: getSingleProductPageRoute ? '111px' : '111px',
                          }}
                        >
